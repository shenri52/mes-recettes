...
def afficher():
    ...
    st.markdown("""
        ...
        /* Style pour réduire la taille des inputs de quantité */
    ...
    # On utilise un dictionnaire local pour pouvoir modifier les quantités du transit avant l'ajout
    if "counts_transit" not in st.session_state or st.session_state.get('last_debut') != debut:
        st.session_state.counts_transit = dict(Counter(liste_brute))
        st.session_state.last_debut = debut
    ...
    if c1.button("⬅️", key="prev_t"): 
        ...
        del st.session_state.counts_transit # Reset pour recalculer
        st.rerun()
    ...
    if st.button("🚀 Actualiser / Classer la liste", use_container_width=True):
        for ing, qte in st.session_state.counts_transit.items():
            zone_dest = st.session_state.index_zones.get(ing)
            if zone_dest:
                z_key = str(zone_dest)
                trouve = False
                for item in st.session_state.data_a5[z_key]["panier"]:
                    if item['nom'] == ing:
                        item['qte'] = str(qte)
                        trouve = True
                        break
                if not trouve:
                    st.session_state.data_a5[z_key]["panier"].append({"nom": ing, "qte": str(qte), "checked": False})
        save_github_data(FILE_PATH, st.session_state.data_a5, st.session_state.sha_a5)
        st.rerun()

    with st.container(border=True):
        items_transit = sorted(st.session_state.counts_transit.items())
        visible_count = 0
        for ing, qte in items_transit:
            ...
            if ing in st.session_state.exclus_transit or dans_panier: continue

            visible_count += 1
            col_nom, col_qte, col_sel, col_add, col_del = st.columns([2, 0.8, 1.2, 0.4, 0.4])
            col_nom.write(ing)
            
            # Modification de la quantité dans le transit
            new_qte = col_qte.text_input("Q", value=str(qte), key=f"q_t_{ing}", label_visibility="collapsed")
            if new_qte != str(qte):
                st.session_state.counts_transit[ing] = new_qte
                st.rerun()

            zone_pref = st.session_state.index_zones.get(ing, "0")
            options_zones = [str(i) for i in range(12)]
            zone_dest = col_sel.selectbox("Z", options_zones, index=int(zone_pref) if zone_pref.isdigit() else 0, key=f"s_{ing}", label_visibility="collapsed", format_func=lambda x: f"Z{int(x)+1}")
            
            if col_add.button("➕", key=f"a_{ing}"):
                st.session_state.data_a5[zone_dest]["panier"].append({"nom": ing, "qte": str(new_qte), "checked": False})
            ...
            if col_del.button("➖", key=f"d_{ing}"):
        ...
        for j in range(2):
            ...
            with cols[j]:
                ...
                with st.container(border=True):
                    ...
                    # AFFICHAGE DU PANIER AVEC QUANTITÉ MODIFIABLE
                    for p_idx, p in enumerate(case["panier"]):
                        c_p1, c_p2, c_p3 = st.columns([2, 1, 0.5])
                        c_p1.markdown(f"**{p['nom']}**")
                        
                        # Input pour modifier la quantité directement dans la zone
                        new_p_qte = c_p2.text_input("Q", value=p['qte'], key=f"q_z_{idx}_{p_idx}", label_visibility="collapsed")
                        if new_p_qte != p['qte']:
                            p['qte'] = new_p_qte
                            save_github_data(FILE_PATH, st.session_state.data_a5, st.session_state.sha_a5)
                            st.rerun()

                        if c_p3.button("🗑️", key=f"del_{idx}_{p_idx}"):
