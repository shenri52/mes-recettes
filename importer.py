...
def afficher():
    ...
    with st.container():
        ...
        with col_cat:
            cat_opts = ["➕ Ajouter une catégorie..."] + sorted(st.session_state.categories_img)
            choix_cat = st.selectbox("Catégorie", options=cat_opts, key=f"cat_sel_{st.session_state.form_count_img}")
        
        with col_cat_new:
            cat_finale = ""
            if choix_cat == "➕ Ajouter une catégorie...":
                nouvelle_cat = st.text_input("Nom nouvelle catégorie", key=f"cat_new_{st.session_state.form_count_img}")
        ...
        with col_ing:
            # Bouton ajouter en haut et liste triée
        ...
        # Ingrédients triés à l'affichage
