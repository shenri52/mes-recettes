...
def afficher():        
    ...
    # Un filtre est considéré actif si on n'est pas sur "Tous"
    ...
    # --- AFFICHAGE DES BOUTONS (Uniquement si filtré pour ne pas polluer) ---
    if filtre_actif:
        if resultats:
            st.write(f"### 📋 {len(resultats)} recette(s) trouvée(s)")
            for r in resultats:
                nom_btn = r['nom'].upper()
                # Si on clique sur le bouton, on met à jour la session et on relance
    ...
    # --- LISTE DÉROULANTE ---
    # On montre tout l'index par défaut, ou seulement les résultats si filtré
    options_select = [r['nom'].upper() for r in (resultats if filtre_actif else index)]
    options = ["---"] + sorted(list(set(options_select)))
    
    # Récupération de la sélection actuelle (bouton ou selectbox)
    ...
    # On synchronise la session avec le choix final
    st.session_state["select_recette"] = choix
    
    # --- AFFICHAGE DE LA FICHE ---
    if choix != "---":
        # On cherche la recette correspondante dans l'index global
        ...
        if info:
            ...
            if res_rec.status_code == 200:
                ...
                if st.session_state[m_edit]:
                    # --- MODE ÉDITION (TES LOGIQUES GARDÉES À 100%) ---
                    ...
                    for idx, item in enumerate(st.session_state[state_key]):
                        ...
                        def_idx = opts_ing.index(current_ing) if current_ing in opts_ing else 0
                        ...
                        if choix_sel == "➕ NOUVEL INGRÉDIENT":
                            nouveau = col_n.text_input("Nom", value=current_ing if current_ing not in opts_ing else "", key=f"new_{item['id']}")
