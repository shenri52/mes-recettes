...
def supprimer_fichier_github(chemin):
    ...
    # Nettoyage du chemin pour GitHub (pas de / au début)
    chemin = chemin.strip("/")
    conf = config_github()
...
def afficher():
    ...
    filtre_ing = col_i.selectbox("Ingrédient", ["Tous"] + sorted(list(set(tous_ings))))
    ...
    for idx, rec in enumerate(recettes_f):
        with st.expander(f"{rec.get('appareil', '🍳')} - {rec.get('nom', 'Sans titre')}"):
            ...
            with c2:
                ...
                if medias:
                    ...
                    for i, m in enumerate(medias):
                        if not m: continue
                        # --- NETTOYAGE DU CHEMIN ---
                        # Si le chemin commence par /, on l'enlève. Si "data/" manque, on l'ajoute.
                        chemin_propre = m.strip("/")
                        if not chemin_propre.startswith("data/"):
                            chemin_propre = f"data/{chemin_propre}"
                        
                        url_api = f"https://api.github.com/repos/{conf['owner']}/{conf['repo']}/contents/{chemin_propre}"
                        res_m = requests.get(url_api, headers=conf['headers'])
                        
                        if res_m.status_code == 200:
                            data = base64.b64decode(res_m.json()['content'])
                            if chemin_propre.lower().endswith('.pdf'):
                                st.download_button(f"📄 PDF {i+1}", data, file_name=chemin_propre.split('/')[-1], key=f"pdf_{idx}_{i}")
                            else:
                                st.image(data, use_container_width=True)
                        else:
                            st.error(f"Fichier non trouvé sur GitHub ({res_m.status_code})")
                            st.caption(f"Chemin testé : {chemin_propre}")
                else:
                    st.write("Aucun média.")import streamlit as st
import requests
import json
import base64
