...
def afficher():        
    ...
    # 1. On détecte si l'utilisateur a utilisé au moins un filtre
    ...
    # --- NOUVEAUTÉ : AFFICHAGE DES BOUTONS (Uniquement si filtré) ---
    if filtre_actif:
        if resultats:
            st.write(f"### 📋 {len(resultats)} recette(s) correspondante(s)")
            for r in resultats:
                nom_btn = r['nom'].upper()
                if st.button(f"📖 {nom_btn}", key=f"btn_filtre_{r['chemin']}", use_container_width=True):
    ...
    # --- TA LISTE DÉROULANTE (Conservée) ---
    # Si on filtre, on montre les résultats, sinon on montre tout l'index
    options_select = [r['nom'].upper() for r in (resultats if filtre_actif else index)]
    options = ["---"] + sorted(options_select)
    
    # On récupère la valeur stockée (qui peut venir de l'URL via app.py ou du clic bouton)
    ...
    # --- LA SUITE : STRICTEMENT TON CODE FOURNI ---
    if choix != "---":
        # Correction vitale ici : on cherche la recette dans l'index global, ce qui empêche le bug du bouton "Modifier"
